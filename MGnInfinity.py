...
class multi_class_MGnInfinity:
    ...
    def simulate_MGnInfinity(self, customers, runs, system_type=1):
        ...
        for r in range(runs):
            ...
            while len(list(event_calendar))>0:
                ...
                if event_ == 'a':
                    ...
                    event_log.append((ts_ + service_time, 'd', id_, interv_[id_], classes_[id_]))
...
if __name__ == "__main__":
    q_ = multi_class_MGnInfinity(lambda_ = 1, classes = [0], probs = [1.0],
                                         mus = [1.1], prob_speedup=[0.5], mus_speedup=[11],
                                         servers = 1)

    q_.simulate_MGnInfinity(customers=5000, runs=3)
    q_.generate_data()
    ...
    # q_2.simulate_q(customers=100, runs=3, system_type=1)
    # q_2.generate_data()